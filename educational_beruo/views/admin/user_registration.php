...
if (isset($_POST['Submit'])) {
	...
	$sql = "INSERT INTO user(full_name, gender, age, phone, address_id) VALUES (?,?,?,?,?)";
  	$result = $db->prepare($sql);
  	$full_name = $_POST['Fname']." ".$_POST['Mname']." ".$_POST['Lname'];
  	$value = array($full_name, $_POST['sex'], $_POST['age'], $_POST['phone'], $address_id);
  	$res = $result->execute($value);
  ...
  $sql = "INSERT INTO users(user_id, role) VALUES (?,?)";
    ...
    $res = $result->execute($value);
	  if($res == true){
	  	...
	  	echo "<script> alert('Sucssfully registerd');</script>";
...
<form  method="post" onsubmit='return  formValidator()'>
	...
	<tr>
      ...
      <td><input type="phone" required  Pattern="[0-9]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="School_Code" name="phone"/></td>
      <td id="LnameError" class="red">&nbsp;</td>
	  </tr>
