...
if (isset($_POST['search']) && !empty($_POST['search'])) {
	$name = $_POST['search'];
	$sql = "SELECT * FROM students st, school sc, address ad, zone z, werda w WHERE full_name LIKE '%$name%' AND st.school_id=sc.id AND st.address_id=ad.id AND z.id=ad.zone_id AND w.id=ad.werda_id AND st.status='Active' AND rstatus='0'";
	$result = $db->query($sql);
	$ro = $result->rowCount();
		if ($ro<1) {
