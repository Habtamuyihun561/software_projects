...
if (isset($_POST['Submit'])) {
	$sql = "INSERT INTO address(werda_id, zone_id) VALUES (?, ?)";
	$result = $db->prepare($sql);
	$value = array($_POST['wereda'], $_POST['zone']);
	$res = $result->execute($value);
	$address_id = $db->lastInsertId();

	$sql = "INSERT INTO school(school_code, school_name, school_type) VALUES (?,?,?)";
	$result = $db->prepare($sql);
	$value = array($_POST['School_Code'], $_POST['School_Name'], $_POST['School_Type']);
	$res = $result->execute($value);
	$school_id = $db->lastInsertId();

	$sql = "INSERT INTO students(full_name, gender, age, registration_no, student_type, address_id, school_id) VALUES (?,?,?,?,?,?,?)";
  	$result = $db->prepare($sql);
  	$full_name = $_POST['Fname']." ".$_POST['Mname']." ".$_POST['Lname'];
  	$value = array($full_name, $_POST['sex'], $_POST['age'], $_POST['Reg_No'], $_POST['Student_Type'], $address_id, $school_id);
  	$res = $result->execute($value);
  	$user_id = $db->lastInsertId();
	if($res == true){
  	$sql = "INSERT INTO users(user_id, role) VALUES (?,?)";
    $result = $db->prepare($sql);
    $value = array($user_id, $_POST['role']);
    $res = $result->execute($value);
...
<input type="hidden" name="role" value="student">
	<tr>
	      <td height="37" align="center">First Name<span class="red">*</span></td>
	      <td><input type="text" required  Pattern="[a-zA-Z]+" onInvalid=setCustomerValidity("Please Enter Only Number")id="Fname" name="Fname" title="Enter First Name"/></td>
	      <td id="FnameError" class="red">&nbsp;</td>
	</tr>
	<tr>
      <td height="37" align="center">Middle Name<span class="red">*</span></td>
      <td><input type="text" required  Pattern="[a-zA-Z]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="Mname" name="Mname"/></td>
      <td id="MnameError" class="red">&nbsp;</td>
	  </tr>
   	 
	<tr>
      <td height="37" align="center">Last Name<span class="red">*</span></td>
      <td><input type="text" required  Pattern="[a-zA-Z]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="Lname" name="Lname"/></td>
      <td id="LnameError" class="red">&nbsp;</td>
	  </tr>
     <tr>
      <td height="37" align="center">Registration Number<span class="red">*</span></td>
      <td><input type="text" required  Pattern="[0-9]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="Reg_No" name="Reg_No"/></td>
      <td id="Reg_NoError" class="red">&nbsp;</td>
	  </tr>
	  <tr>
     <td height="37" align="center">Age<span class="red">*</span></td>
      <td><input type="Number" required  Pattern="[0-9]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="age" max="100" min="12" name="age"/></td>
	...
	<tr>
      ...
      <td><input type="text" required  Pattern="[a-zA-Z]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="School_Name" name="School_Name"/></td>
	...
	<tr>
      ...
      <td><input type="text" required  Pattern="[0-9]+" onInvalid=setCustomerValidity("Please Enter Only Number") id="School_Code" name="School_Code"/></td>
      <td id="LnameError" class="red">&nbsp;</td>
	  </tr>
